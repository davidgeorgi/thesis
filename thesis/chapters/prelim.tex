...
However, if the prediction performance is high on the training set, but not reliable on unseen data, the hypothesis \textit{over fits} the training data.
In this cases the model complexity is usually too high. 
In contrast, if the model is too simple to fit any data from $f(x)$, the hypothesis is \textit{underfitting}.
In many real-world applications, the true function $f(X)$ is stochastic, i.e. we need to estimate a conditional probability function $P(Y | X)$ (classification problem) or a conditional expectation $E(Y | X)$ (regression problem).
Therefore, the prediction accuracy is limited by the variance of the true function $f(X)$.
