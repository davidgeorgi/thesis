...
These texts in natural language might hold process-critical information and should consequently be consider for process prediction.
...
\begin{cases}
	\text{PROCESS END}& \text{if $k = |\sigma|$} \\
...
In some applications the case outcome is defined by the activity of activity of the final event of a case.
...
These functions are approximated via an LSTM model using historical event log data, i.e. a set of completed traces with full information about the course of the process instances.
...
Due to the probabilistic behavior of real-life processes, the prediction accuracy is always limited by the randomness of the process behavior.
...
Given an event log $\eventlog = \{\sigma_1, \dots, \sigma_l\}$ with completed traces, the set of all prefix traces $\eventlog_\mathrm{prefix} = \{ hd^k(\sigma) \mid  \sigma \in \eventlog, 1 \leq k \leq |\sigma|\}$ is computed and each trace is encoded as a sequence of event vectors.
...
\begin{table}[!htbp]
	...
	\begin{tabularx}{\textwidth}{p{2cm} l l p{6.8cm} }
		...
		$\vec{d}_{ij}^\mathrm{num}$ &  $\hat{\pi_{\mathcal{D}_j^\mathrm{num}}(e_i)} $ &1 & Normalized value of the $j$-th numerical attribute\\
		...
		$\vec{d}_{ij}^\mathrm{text}$ & See Section \ref{sec:text} & $z_i$& Fixed-length vectorization of the $j$-th text attribute.\\
		\bottomrule
	\end{tabularx}
...
\end{equation*}
An overview of each feature vector is provided in Table \ref{tab:features}.
...
The vocabulary of a text corpus is a set $V$ of all relevant words that appear in the corpus and is indexed by a bijective index function $index_V \colon V \to \{1, 2, \dots, |V|\}$.
As text vectorization techniques the Bag of Words, Bag of N-Grams, Paragraph Vector and Latent Dirichlet Allocation text models are considered.
...
However, the dimension can be reduced by limiting the size of the vocabulary. For example, words with low tf-idf scores can excluded from the vocabulary.
...
This guarantees a continuous forward-projection on running process instances, that is updated whenever new information becomes available.